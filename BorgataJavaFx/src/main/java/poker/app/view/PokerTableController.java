...
public class PokerTableController {
	...
	//Determines gameToPlay, gets value from the radio buttons
	// See RootLayoutController.Java for implementation
	public static int gameToPlay;
	...
	private void handlePlay() {
		...
		// Determine what game is selected
		//gameToPlay = RootLayoutController.gameSelected();
		...
		// Initialize the rule variable
		Rule rle = null;
		
		// Switch statement determining game type, defaults to FiveStud
		// Gets the value from radio button FXML methods at the bottom of RootLayoutController.Java
		switch(gameToPlay) {
		case 1:
	...
	private void handleDraw() {
		...
		if (act.isbEvaluateHand()) {
			...
			case 1:
				winnerName = this.txtP1Name.getText();
				break;
			case 2:
				winnerName = this.txtP2Name.getText();
				break;
			case 3:
				winnerName = this.txtP3Name.getText();
				break;
			case 4:
				winnerName = this.txtP4Name.getText();
			}
			
			Alert winner = new Alert(AlertType.INFORMATION);
			winner.setTitle("Result of Game");
			winner.setHeaderText("Winning Player was: " + winnerName);
